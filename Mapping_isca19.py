...
#     total_hops_between_fso_and_ssdgpu = (ssd_run+gpu_run)*(torus_two_y+1)
    ...
    dataframe.to_csv("results/job_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[-1] + ".csv", index = False, sep = ',')    
...
#     f = open("stat_su", "w") #system utilization
    ...
    dt = dt.replace(":", "-")
    ar = MyPWA.archive.replace(".", "-")
    fn = "results/" + dt + "_" + GUI.topo + "_" + str(tn) + "_" + arch + "_" + GUI.schedule + "_" + GUI.mode + "_" + str(GUI.fso_r) + "_fsoconfigtime_" + str(fso_config_time) + "_fso_cpu_ssdgpu_" + str(fso_cpu_ssdgpu) + "_" + GUI.speedup + "_" + ar.split("/")[-1] #file name
...
#             average_utilization = total_utilization/ts
            ...
            dataframe.to_csv("results/system_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[-1] + ".csv", index = True, sep = ',')
            
            dataframe = pd.DataFrame({"switches": jobs_cpus_sws, "hosts": jobs_cpus_nodes, "number of nodes": jobs_cpus})
            dataframe.to_csv("results/mapping_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[-1] + ".csv", index = True, sep = ',')
