...
if (GUI.topo == "random-regular"):
    RG = nx.random_regular_graph(6, 3072)
    for node in RG.nodes():
        RG.node[node]["cpu"] = 1
        RG.node[node]["ssd"] = 0
        RG.node[node]["gpu"] = 0
...
def output_queue_time(fn):
    ...
    dataframe.to_csv("results/job_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[-1] + ".csv",
                     index=False, sep=',')
...
def dostat():
    ...
    while (True):
        ...
        if (len(queue) == 0 and all_submitted == True and total_cpu == tn):
            ...
            dataframe.to_csv(
                "results/system_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[-1] + ".csv",
                index=True, sep=',')

            dataframe = pd.DataFrame(
                {"switches": jobs_cpus_sws, "hosts": jobs_cpus_nodes, "number of nodes": jobs_cpus})
            dataframe.to_csv(
                "results/mapping_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[-1] + ".csv",
                index=True, sep=',')
