...
for i in range(num_jobs):
    ...
    if (MyPWA.archive.split(".")[-1] == "swf"):
        jobs[i] = (data["Number of Allocated Processors"][i], 0, 0, data["Submit Time"][i] / k, data["Run Time"][i] / k,
                   data["Job Number"][i])
    else:
        jobs[i] = (
        data["cpu#"][i], data["ssd#"][i], data["gpu#"][i], data["submit"][i] / k, data["runtime"][i], data["npb"][i])
                            ...
                            for each in fnl:
                                ...
                                for n in RG_.nodes():
                                    ...
                                    if (n == RG_.nodes()[-1]):
                                        #                                     print "3"
                                        jump_3 = jump_3 + 1
                                        if (fill == 1 and RG.node[nouse[0]]["cpu"] > 0 and RG.node[nouse[0]]["fso"] >=
                                                each[1]):
...
def output_queue_time(fn, dt):
    ...
    if arch == "IRS-REPEAT":
        if GUI.mode == "FSO":  # fat-tree
            ...
            avg_hops_between_fso_and_ssdgpu = (cpu_ssdgpu_improved_by_fso * 2 + (
                        ssd_run + gpu_run - cpu_ssdgpu_improved_by_fso) * (torus_three_z + 1)) / float(
                ssd_run + gpu_run)
            f.write("fso betweeen cpu and ssd/gpu: " + str(fso_cpu_ssdgpu) + "\n")
            f.write("improved hops between cpu/ssd(gpu) by fso: " + str(cpu_ssdgpu_improved_by_fso) + " / " + str(
                ssd_run + gpu_run) + " = " + str(float(cpu_ssdgpu_improved_by_fso) / (ssd_run + gpu_run)) + "\n")
    ...
    dataframe = pd.DataFrame({'jobs': [num_jobs], 'dispatched jobs': [num], 'avg_queue_time': [avg_queue_time],
                              'avg_response_time': [avg_response_time], 'avg_slowdown': [avg_slowdown],
                              'switches': [host_start], 'hosts': [tn], 'scheduling': ["backfilling"],
                              'embedding': ["connected"], 'edgelist': [MyPWA.edgelist.split("/")[-1]],
                              'workload': [MyPWA.archive.split("/")[-1]], 'scale': [1 / k],
                              'simulation time': [end - start], 'failed jobs': [len(failed_jobs)],
                              'avg_node': [avg_node], 'avg_diameter': [avg_diameter], 'avg_aspl': [avg_aspl]})
    dataframe.to_csv(
        "results/job_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[-1] + "_" + dt + ".csv",
        index=False, sep=',')
...
def dostat():
    ...
    dt = dt.replace(":", "-")
    ar = MyPWA.archive.replace(".", "-")
    fn = "results/" + dt + "_" + GUI.topo + "_" + str(
        tn) + "_" + arch + "_" + GUI.schedule + "_" + GUI.mode + "_" + str(GUI.fso_r) + "_fsoconfigtime_" + str(
        fso_config_time) + "_fso_cpu_ssdgpu_" + str(fso_cpu_ssdgpu) + "_" + GUI.speedup + "_" + ar.split("/")[
             -1] + ".txt"  # file name
    ...
    while (True):
        ...
        s = str(ts * timestep) + "    " + str(tn - total_cpu) + "    " + str(tn) + "    " + str(uti_cpu) + "    " + str(
        ...
        if (len(queue) == 0 and all_submitted == True and total_cpu == tn):
            ...
            dataframe.to_csv("results/system_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[
                -1] + "_" + dt + ".csv", index=True, sep=',')

            dataframe = pd.DataFrame({"switches": jobs_cpus_sws, "hosts": jobs_cpus_nodes, "number of nodes": jobs_cpus,
                                      "diameter": jobs_cpus_diameter, "aspl": jobs_cpus_aspl})
            dataframe.to_csv("results/mapping_" + MyPWA.edgelist.split("/")[-1] + "_" + MyPWA.archive.split("/")[
                -1] + "_" + dt + ".csv", index=True, sep=',')
...
while (True):
    if (len(queue) > 0):
        ...
        if (torus_d == 3 and (
                GUI.speedup == "sub2dmesh" or GUI.speedup == "sub3dmesh" or GUI.speedup == "sub3dtorus")):  #
            ...
            if transform == True:
                dilation += 1
                if dilation > th or (g[0] + (g[0] - 1) * (dilation - 1) > torus_three_x or g[1] + (g[1] - 1) * (
                        dilation - 1) > torus_three_y or g[2] + (g[2] - 1) * (dilation - 1) > torus_three_z):
                    dilation = 1
                    reset()
                    to_first = False
                    continue
                ...
                for b in range(torus_three_y):
                    for a in range(torus_three_x):
                        ...
                        if (flag == True):
                            ...
                            n = 0
                            if arch == "RS":
                                for aaa in range(a, a + g[0] + (g[0] - 1) * (dilation - 1),
                                                 dilation):  # ergodic in x*y grid
                                    for bbb in range(b, b + g[1] + (g[1] - 1) * (dilation - 1), dilation):
                                        for ccc in range(c, c + g[2] + (g[2] - 1) * (dilation - 1), dilation):
                                            aaaa = aaa % torus_three_x
                                            bbbb = bbb % torus_three_y
